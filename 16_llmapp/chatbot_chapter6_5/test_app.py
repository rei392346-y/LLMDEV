...
from chatbot_chapter6_5.graph import memory, get_messages_list

@pytest.fixture
def client():
    """
    Flaskテストクライアントを作成
    ・Flask アプリケーションのテスト用クライアントを作成します。
    ・TESTING モードを有効化して、エラーのデバッグを簡単にします。
    """
...
def test_index_get_request(client):
    ...
    ・初期画面が正しくレンダリングされるか確認します。
    ・メモリがリセットされていることも検証します。
    """
    with client as c:
        response = c.get('/')
...
def test_index_post_request(client):
    ...
    ・ユーザー入力の内容が HTML に正しく表示されるかを確認します。
    ・ボットの応答も HTML にレンダリングされるべきです。
    """
    user_message = "1たす2は？"
...
def test_memory_persistence(client):
    ...
    ・複数のリクエストを送信した際に、メモリがそれぞれのメッセージを保持しているかを検証します。
    """
...
def test_template_rendering(client):
    ...
    ・Flask テンプレートエンジンで、ユーザーとボットのメッセージが正しく表示されるかを検証します。
    """
    user_message = "1たす2は？"
