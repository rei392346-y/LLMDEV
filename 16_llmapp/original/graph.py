...
def build_graph(model_name, memory, system_prompt=None):
    ...
    # ツールノードの作成
    """
    ツール定義を関数化したことにより、build_graph 関数のなかでの tools の作成を関数呼び出しに変更しています。

セッションごとにメモリを保持（複数人の利用に対応）
    これまでは "1" 固定だった thread_id を、パラメータで渡した値を設定するように変更しています。    
    ・stream_graph_updates 関数
    ・get_bot_response 関数
    ・get_messages_list 関数
    """
    tools = define_tools()
