...
USER_MESSAGE_2 = "博多駅のイベントの検索結果を教えて"
USER_MESSAGE_3 = "LESSON17の内容は？"
...
def test_memory_persistence_with_session(client):
    ...
    client.post('/', data={'user_message': USER_MESSAGE_3})
    ...
    assert any("博多駅" in msg['text'] for msg in messages if msg['class'] == 'user-message'), "メモリに2番目のユーザーメッセージが保存されるべきです。"
    assert any("LESSON17" in msg['text'] for msg in messages if msg['class'] == 'user-message'), "メモリに3番目のユーザーメッセージが保存されるべきです。"
