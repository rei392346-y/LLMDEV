...
セッション (session) の利用をテスト:
・client.session_transaction() を使用してセッションにアクセスし、thread_id の設定や削除を確認しています。

/clear エンドポイントのテストを追加:
・/clear を呼び出した後、セッションがクリアされていること（thread_id が削除されていること）を確認します。
・メモリがリセットされ、以前のデータが残っていないことも確認します。

POST リクエストでの thread_id の動作確認:
・初回の POST リクエストでセッションに thread_id が生成されることを確認します。
・thread_id がメモリのメッセージ保存に正しく使用されていることを検証します。
"""
import pytest
